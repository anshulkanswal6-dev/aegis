    ...
    <div className="space-y-14">
      ...
      <div>
        ...
        <div className="space-y-3">
          {[
            { icon: Fingerprint, title: 'Wallet Authentication', desc: 'Cryptographic wallet signature provides authentication without credential storage. Session management is handled client-side through the wallet provider. No sensitive authentication data is stored on the backend.', accent: 'border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400' },
            { icon: Lock, title: 'Resource Ownership Scoping', desc: 'All database queries for user-facing resources include user_id/wallet_address filtering. Projects, automations, and execution history are strictly scoped to the authenticated wallet. Cross-user resource access is architecturally prevented at the query layer.', accent: 'border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950 text-violet-600 dark:text-violet-400' },
            { icon: Shield, title: 'Agent Wallet Isolation', desc: 'AEGIS implements a smart contract-based Agent Wallet system via a Factory contract pattern. The Agent Wallet is a separate on-chain entity from the user\'s primary wallet, providing transaction isolation. Funds deposited to the Agent Wallet are operationally separated from the user\'s main holdings.', accent: 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400' },
            { icon: Key, title: 'Integration Link Verification', desc: 'Telegram account linking uses a one-time token-based verification flow. Tokens are generated server-side, transmitted to the user through the dashboard, and verified by the Telegram bot upon receipt. Each token is single-use with conflict detection for already-linked accounts.', accent: 'border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950 text-amber-600 dark:text-amber-400' },
            { icon: Eye, title: 'Audit Trail', desc: 'Every automation execution, state transition, and operational action is logged with timestamps and contextual metadata. Execution logs are append-only with no deletion pathway. Run history provides a complete auditable record of all system operations.', accent: 'border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-950 text-rose-600 dark:text-rose-400' },
            { icon: Network, title: 'API Security', desc: 'Backend API endpoints implement validation at multiple layers. Request parameters are sanitized, UUID values are verified, and database operations use parameterized queries through the PostgREST layer to prevent injection attacks.', accent: 'border-cyan-200 dark:border-cyan-800 bg-cyan-50 dark:bg-cyan-950 text-cyan-600 dark:text-cyan-400' },
